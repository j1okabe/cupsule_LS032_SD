...
modified by @jokabe j1okabe@gmail.com
for LS032B7DH01 10bit aqddress device
...
void Adafruit_SharpMem::refresh(void)
{
  ...
  myCmd.word_data = _sharpmem_vcom | SHARPMEM_BIT_WRITECMD | (linecount << 6);

  spidev->transfer(myCmd.data, 2);
  TOGGLE_VCOM;
  // Create a buffer for the entire display
  totalbytes = HEIGHT * (bytes_per_line + 2);
  ...
  for (i = 0; i < HEIGHT; i++)
  {
    ...
    memcpy(buf_ptr, sharpmem_buffer + (i * bytes_per_line), bytes_per_line);

    myCmd.word_data = SHARPMEM_BIT_WRITECMD | (linecount << 6);
    buf_ptr[bytes_per_line + 0] = (uint8_t)(myCmd.data[0]);
    buf_ptr[bytes_per_line + 1] = (uint8_t)(myCmd.data[1]);

    buf_ptr += (bytes_per_line + 2);
    linecount++;
  }

  // Send all pixels data
  spidev->transfer(buffer, totalbytes);
  delete[] buffer;
  digitalWrite(_cs, LOW);
  // spidev->endTransaction();
}
